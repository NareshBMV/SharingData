...
class GenerateInvoiceController: BaseViewController,IndicatorInfoProvider,UITableViewDataSource, UITableViewDelegate {
    ...
    @IBAction func generateInvoiceAction(_ sender: Any) {
        ...
        SharedData.sharedInstance.totalCost = totalCost.text!
        SharedData.sharedInstance.invoiceNumber = String(invoiceNumber)
        SharedData.sharedInstance.newInvoiceSelectedProducts.removeAll()
        SharedData.sharedInstance.newInvoiceSelectedProducts.append(contentsOf: SharedData.sharedInstance.selectedProducts)
